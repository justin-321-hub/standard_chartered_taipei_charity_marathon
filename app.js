  ...
  try {
    ...
    if (commonHttpErrors.includes(res.status)) {
      if (retryCounts.httpErrors === 0) {
        ...
        throw new Error("抱歉，網路不穩定，請稍後再試。");
      }
    }
    ...
    if (typeof data === "string") {
      replyText = data.trim() || "請您重新表達您的問題，謝謝。";
    } else if (data && typeof data === "object") {
      const hasTextField = 'text' in data || 'message' in data;
      if (hasTextField) {
        const textValue = data.text !== undefined ? data.text : data.message;
        if (textValue === "" || textValue === null || textValue === undefined) {
          replyText = "請您重新表達您的問題，謝謝。";
        } else {
          replyText = String(textValue).trim() || "請您重新表達您的問題，謝謝。";
        }
      } else {
        const isPlainEmptyObject =
          !Array.isArray(data) &&
...
messages.push({
  id: uid(),
  role: "assistant",
  text: "<p>親愛的跑者您好!</p><p>我是<strong>2026 101 垂直馬拉松</strong>線上客服小幫手，需要查詢甚麼資訊呢?</p><p>歡迎提問哦！</p>",
  ts: Date.now(),
});
render();
