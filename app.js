...
async function sendText(text) {
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 502 || res.status === 404) throw new Error("網路不穩定，請再試一次!");
    ...
    if (typeof data === "string") {
      ...
      replyText = isPlainEmptyObject ? "網路不穩定，請再試一次" : JSON.stringify(data, null, 2);
    }
